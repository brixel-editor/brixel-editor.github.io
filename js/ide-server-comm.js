...
window.IDEServerComm = {
    ...
    async compileCode() {
        ...
        // 🔥 추가: 컴파일 전 블록 검증 (Phase 3)
        if (typeof window.validateBlockAssembly === 'function') {
            const validation = window.validateBlockAssembly();
            
            if (!validation.valid) {
                validation.errors.forEach(error => {
                    // 심각한 오류만 컴파일 중단
                    if (error.type === 'incompatible_blocks' || error.type === 'duplicate_init') {
                        this.logAndSend('❌ ' + error.message);
                    } else {
                        // 경고는 출력만 하고 계속 진행
                        this.logAndSend('⚠️ ' + error.message);
                    }
                });
                
                // 심각한 오류가 있으면 컴파일 중단
                const criticalErrors = validation.errors.filter(e => 
                    e.type === 'incompatible_blocks' || e.type === 'duplicate_init'
                );
                
                if (criticalErrors.length > 0) {
                    this.logAndSend(
                        window.IDEI18n ? 
                        window.IDEI18n.getMsg('compile_validation_failed', '❌ 블록 검증 실패. 위의 오류를 수정해주세요.') : 
                        '❌ 블록 검증 실패. 위의 오류를 수정해주세요.'
                    );
                    return; // 컴파일 중단
                }
            }
        ...
        try {
            ...
            if (!response.ok) {
                ...
                console.error('컴파일 실패 상세:', errorData);
