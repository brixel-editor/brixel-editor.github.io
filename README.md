...
Brixel WebEditor is a powerful web-based block coding platform that allows users to code and upload to various microcontroller boards, including **Arduino**, **ESP32**, and **Raspberry Pi Pico**, from a single unified interface. With **500+ blocks**, **23 languages**, and **AI assistance**, it provides everything needed for physical computing education.

> **Version**: 2.7.0 | **Website**: [brixel.gorillacell.kr](https://brixel.gorillacell.kr)
...
### 2. 500+ Pre-Built Blocks (24 Categories)
...
| **Motors** | Servo, Geek Servo360 (5Kg, gray, orange), Green GeekServo, DC Motor (L298N), PCA9685 (16ch PWM), AccelStepper, Multi-Stepper |
...
- **Block Catalog Awareness**: AI understands all 500+ available blocks for accurate suggestions

### 5. 23 Languages (i18n)
All UI text and block labels fully translated in **1,763 keys** across 23 languages:
...
│   ├── brixel_blocks_definitions.js  # 500+ block definitions (JSON)
...
│   ├── ide-i18n.js              # Internationalization (23 languages)
...
│   └── ui_i18n/                 # 23 language JSON files (1,763 keys each)
...
1. Open [brixel.gorillacell.kr](https://brixel.gorillacell.kr) in your browser
...
브릭셀 웹에디터는 아두이노(Arduino), ESP32, 라즈베리 파이 피코(Raspberry Pi Pico) 등 다양한 보드를 하나의 인터페이스에서 코딩하고 업로드할 수 있는 웹 기반 블록 코딩 플랫폼입니다. **500개 이상의 블록**, **23개 언어**, **AI 도우미**를 제공하여 피지컬 컴퓨팅 교육에 필요한 모든 것을 갖추고 있습니다.

> **버전**: 2.7.0 | **웹사이트**: [brixel.gorillacell.kr](https://brixel.gorillacell.kr)
...
| **모터** | 서보, Geek Servo360(5Kg, gray, orange), 그린 긱서보, DC 모터(L298N), PCA9685(16ch PWM), AccelStepper, 멀티 스텝모터 |
...
- **블록 카탈로그 인식**: 500개 이상의 블록을 모두 인식하여 정확한 추천

### 5. 23개 언어 지원 (i18n)
모든 UI 텍스트와 블록 레이블이 **1,763개 키**에 걸쳐 23개 언어로 완전 번역:
...
### 웹 버전 (설치 불필요)
1. 브라우저에서 [brixel.gorillacell.kr](https://brixel.gorillacell.kr) 접속
